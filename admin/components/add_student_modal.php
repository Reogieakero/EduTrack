            ...
            <div class="pt-4">
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        <button type="button" id="singleEnrollTab" data-tab="single" class="tab-button whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm border-primary-green text-primary-green">
                            Single Enrollment
                        </button>
                        <button type="button" id="bulkEnrollTab" data-tab="bulk" class="tab-button whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                            Bulk Enrollment
                        </button>
                    </nav>
                </div>
                ...
                <div id="singleEnrollContent" class="tab-content pt-6 space-y-5">
                    <form method="POST" action="students.php" class="space-y-5" id="addStudentForm">
                            ...
                            <div id="custom-section-select" class="relative">
                                ...
                                <ul id="section-options-list" 
                                    ...
                                    <li class="py-2 px-3 cursor-pointer text-gray-900 text-sm transition-colors duration-150 hover:bg-gray-100"
                                        ...
                                        onclick="handleSectionSelect(this, 'add')">
                ...
                <div id="bulkEnrollContent" class="tab-content pt-6 space-y-5 hidden">
                    <form method="POST" action="students.php" enctype="multipart/form-data" class="space-y-5" id="bulkAddStudentForm">
                        ...
                        <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                            ...
                            <ul class="list-disc list-inside space-y-1">
                                <li>The file must be a CSV or Excel (.xlsx, .xls) file.</li>
                                <li>The first row must be a header row and will be skipped.</li>
                                <li>Required columns (in order): First Name, Last Name, Middle Initial (optional), Section (Year - Name), Date of Birth (YYYY-MM-DD).</li>
                                <li>Example Section format: *Year 7 - Diamond*</li>
                                <li>*Note: If using an Excel file, ensure the Date of Birth column is formatted as Date.*</li>
                            </ul>
                        </div>
                        
                        <div>
                            <label for="student_file" class="block text-sm font-medium text-gray-700 mb-1">Upload CSV or Excel File</label>
                            <input type="file" id="student_file" name="student_file" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" required class="w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary-blue file:text-white hover:file:bg-blue-700">
                        </div>

                        <div class="pt-4 border-t flex justify-end">
                            <button type="submit" id="saveBulkStudentBtn" class="w-full sm:w-auto bg-primary-blue hover:bg-blue-700 text-white font-semibold py-2.5 px-6 rounded-lg transition duration-150 shadow-md flex items-center justify-center space-x-2">
                                <i data-lucide="upload-cloud" class="w-5 h-5" id="saveBulkIcon"></i>
                                <span id="saveBulkText">Upload & Enroll Students</span>
                                <svg id="loadingBulkSpinner" class="animate-spin h-5 w-5 text-white hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                            </button>
